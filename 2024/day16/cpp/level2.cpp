...
int get_best_score(vector<string> maze, ReindeerStep start, Position end) {
    ...
    while (!is_finished(opened_set,end)) {
        ...
        ReindeerStep new_step = {{explored_step.position.x+dx,explored_step.position.y+dy},explored_step.direction,explored_step.score + 1,explored_step.positions};
        new_step.positions.insert(explored_step.position);
        if (new_step.position.x == end.x && new_step.position.y == end.y) {
            return new_step.positions.size()+1;
        }
        if (maze[new_step.position.y][new_step.position.x] != '#' && !closed_set.contains(new_step.position)) {
            opened_set.insert(new_step);
        } else if (maze[new_step.position.y][new_step.position.x] != '#' && opened_set.find(new_step) != opened_set.end()) {
            auto similar = opened_set.find(new_step);
            for (Position pos : similar->positions) {
                new_step.positions.insert(pos);
            }
            opened_set.erase(similar);
            opened_set.insert(new_step);
        }
        auto[dx_cw,dy_cw] = turn_clockwise(explored_step.direction);
        ReindeerStep new_step_cw = {{explored_step.position.x,explored_step.position.y},{dx_cw,dy_cw},explored_step.score + 1000,explored_step.positions};
        ...
        ReindeerStep new_step_ccw = {{explored_step.position.x,explored_step.position.y},{dx_ccw,dy_ccw},explored_step.score + 1000,explored_step.positions};
        ...
        if (n_loops%10==0)cout << n_loops << endl;
        n_loops ++;
    ...
    return (*opened_set.begin()).positions.size()+1;
